...
class NMT(nn.Module):
  ...
  def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
    ...
    self.target_vocab_projection = nn.Linear(
        in_features=hidden_size, out_features=len(vocab.tgt))
  ...
  def encode(self, source_padded: torch.Tensor, source_lengths: List[int]
    ...
    # h0, c0 default to zero vectors if not provided.
  ...
  def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
    ...
    for Y_t in torch.split(Y, split_size_or_sections=1):
      ...
      desc_state, o_t, _, = self.step(Ybar_t, dec_state, enc_hiddens,
  ...
  def step(
      self, Ybar_t: torch.Tensor, dec_state: Tuple[torch.Tensor, torch.Tensor],
  ...
  def beam_search(self,
    ...
    while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
      ...
      y_tm1 = torch.tensor([self.vocab.tgt[hyp[-1]] for hyp in hypotheses],
                           dtype=torch.long,
                           device=self.device)
